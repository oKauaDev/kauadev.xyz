...
const ButtonAction: React.FC<ButtonActionProps> = ({
  ...
  return (
    <button
      onClick={handleClick}
      className={`block w-max bg-primary-100 text-primary-400 uppercase px-5 py-2 font-bold rounded leading-4 text-base cursor-pointer border-2 border-transparent border-solid transition-all duration-300 hover:bg-transparent hover:border-primary-100 ${className}`}
      {...args}
    >
      {children}
    </button>
